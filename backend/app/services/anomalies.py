...
def flagged_transaction_ids(db: Session, user_id: int) -> set[int]:
    """Ids of every debit the detector flags across the user's whole history."""
    df, feats, result, debit = _score_user(db, user_id)
    if df is None:
        return set()
    return {int(df.at[i, "id"]) for i in result.index[result["is_anomaly"]]}


def _score_user(db: Session, user_id: int):
    """(transactions, features, detector output for debits, debit mask), or Nones when data is too thin."""
    stmt = select(
    ...
    if debit.sum() < MIN_DEBITS_FOR_DETECTION:
        return None, None, None, None
    return df, feats, get_detector().flag(feats[debit], df.loc[debit, "user_id"]), debit
